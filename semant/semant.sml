    ...
    and validateEquality(left, right, pos, oper) =
      ...
      in
        case actual_ty(tenv,(#ty l),pos) of
            Types.INT => if checkInt(r, pos, true) then {exp=TR.intBinOps(oper, #exp l, #exp r), ty=Types.INT} else (handleFail(pos, "Error: Compared structures must be of same type \n"); {exp=TR.handleNil(), ty=Types.BOTTOM})
          | Types.STRING => if checkStr(r, pos, true) then {exp=TR.strBinOps(oper, #exp l, #exp r), ty=Types.INT} else (handleFail(pos, "Error: Compared structures must be of same type \n"); {exp=TR.handleNil(), ty=Types.BOTTOM})
          | Types.ARRAY(ty', unique') => if checkSameType(Types.ARRAY(ty', unique'), #ty r) then {exp=TR.intBinOps(oper, #exp l, #exp r), ty=Types.INT} else (handleFail(pos, "Error: Compared structures must be of same type \n"); {exp=TR.handleNil(), ty=Types.BOTTOM})
          | Types.RECORD(fields, unique') => if checkSameType(Types.RECORD(fields, unique'), #ty r) then {exp=TR.intBinOps(oper, #exp l, #exp r), ty=Types.INT} else (handleFail(pos, "Error: Compared structures must be of same type \n"); {exp=TR.handleNil(), ty=Types.BOTTOM})
          | Types.NIL => if checkSameType(Types.NIL, #ty r) then {exp=TR.handleInt 1, ty=Types.INT} else (handleFail(pos, "Error: Compared structures must be of same type \n"); {exp=TR.handleNil(), ty=Types.BOTTOM})
  			  ...
  			  | trvar (A.FieldVar(v, id, pos)) =
							...
							in
								(case ty of Types.RECORD(stl, u) => let fun searchField ((s,t)::m) id = if s = id then actual_ty (tenv,t,pos) else searchField m id
															 ...
															 | _ => (handleFail(pos, "Error: Variable is not defined as a record: "^"\n"))
															)
							end
	  ...
	  | transDec (venv, tenv, A.FunctionDec(l), lev, breakpoint) =
		let
			...
			fun passHeader ({name,params,body,pos,result}, {venv=venv,tenv=tenv}) =
				if valOf(mymap.find(namemap,name)) then (handleFail(pos, "Error: Function name redefined " ^ Symbol.name name^"\n");{venv=venv,tenv=tenv})
				else (
					let
						...
						fun transparam {name, escape, typ, pos} =
												case Symbol.look(tenv,typ)
													...
													| NONE => (handleFail(pos, "Error: Undefined parameter type " ^ Symbol.name name^"\n");
