...
|  ( 4, ( ( _, ( MlyValue.ntVOID lvalue1, lvalue1left, lvalue1right))
 ...
 in (
if isSome (look(table, A.symbol lvalue)) then valOf look(table, A.symbol lvalue) else raise ERROR 
)
end; ()))
...
MlyValue.ntVOID (fn _ => ( let val  (exp as exp1) = exp1 ()
 in (
A.OpExp{left = 0   , oper = A.MinusOp,  right = exp,                 pos = MINUSleft}
)
end; ()))
 in ( LrTable.NT 0, ( result, MINUS1left, exp1right), rest671)
end
|  ( 6, ( ( _, ( MlyValue.ntVOID exp2, _, exp2right)) :: ( _, ( _, 
PLUSleft, _)) :: ( _, ( MlyValue.ntVOID exp1, exp1left, _)) :: rest671
...
MINUSleft, _)) :: ( _, ( MlyValue.ntVOID exp1, exp1left, _)) :: 
rest671)) => let val  result = MlyValue.ntVOID (fn _ => ( let val  
exp1 = exp1 ()
 val  exp2 = exp2 ()
 in (
...
TIMESleft, _)) :: ( _, ( MlyValue.ntVOID exp1, exp1left, _)) :: 
rest671)) => let val  result = MlyValue.ntVOID (fn _ => ( let val  
exp1 = exp1 ()
 val  exp2 = exp2 ()
 in (
...
DIVIDEleft, _)) :: ( _, ( MlyValue.ntVOID exp1, exp1left, _)) :: 
rest671)) => let val  result = MlyValue.ntVOID (fn _ => ( let val  
exp1 = exp1 ()
 val  exp2 = exp2 ()
 in (
...
ANDleft, _)) :: ( _, ( MlyValue.ntVOID exp1, exp1left, _)) :: rest671)
) => let val  result = MlyValue.ntVOID (fn _ => ( let val  exp1 = exp1
 ()
 val  exp2 = exp2 ()
 in (
A.IfExp{test = exp1, then' = exp2,      else' = SOME 0,              pos = ANDleft}
)
end; ()))
 in ( LrTable.NT 0, ( result, exp1left, exp2right), rest671)
end
|  ( 11, ( ( _, ( MlyValue.ntVOID exp2, _, exp2right)) :: ( _, ( _, 
ORleft, _)) :: ( _, ( MlyValue.ntVOID exp1, exp1left, _)) :: rest671))
 => let val  result = MlyValue.ntVOID (fn _ => ( let val  exp1 = exp1
 ()
 val  exp2 = exp2 ()
 in (
A.IfExp{test = exp1, then' = 1,         else' = SOME exp2,           pos = ORleft}
)
end; ()))
 in ( LrTable.NT 0, ( result, exp1left, exp2right), rest671)
end
|  ( 12, ( ( _, ( MlyValue.ntVOID exp2, _, exp2right)) :: ( _, ( _, 
EQleft, _)) :: ( _, ( MlyValue.ntVOID exp1, exp1left, _)) :: rest671))
 => let val  result = MlyValue.ntVOID (fn _ => ( let val  exp1 = exp1
 ()
 val  exp2 = exp2 ()
 in (
...
NEQleft, _)) :: ( _, ( MlyValue.ntVOID exp1, exp1left, _)) :: rest671)
) => let val  result = MlyValue.ntVOID (fn _ => ( let val  exp1 = exp1
 ()
 val  exp2 = exp2 ()
 in (
...
LTleft, _)) :: ( _, ( MlyValue.ntVOID exp1, exp1left, _)) :: rest671))
 => let val  result = MlyValue.ntVOID (fn _ => ( let val  exp1 = exp1
 ()
 val  exp2 = exp2 ()
 in (
...
GTleft, _)) :: ( _, ( MlyValue.ntVOID exp1, exp1left, _)) :: rest671))
 => let val  result = MlyValue.ntVOID (fn _ => ( let val  exp1 = exp1
 ()
 val  exp2 = exp2 ()
 in (
...
explist1, _, _)) :: _ :: ( _, ( MlyValue.ID ID1, (IDleft as ID1left),
 ...
 in (
A.CallExp{func = A.symbol ID,      args = explist,                   pos = IDleft}
)
end; ()))
...
MlyValue.ntVOID (fn _ => ( let val  (ID as ID1) = ID1 ()
 in (
A.CallExp{func = A.symbol ID,      args = [],                        pos = IDleft}
)
end; ()))
 in ( LrTable.NT 0, ( result, ID1left, RPAREN1right), rest671)
end
...
field1, _, _)) :: _ :: ( _, ( MlyValue.ID ID1, (IDleft as ID1left), _)
...
MlyValue.ntVOID (fn _ => ( let val  (ID as ID1) = ID1 ()
 ...
 val  exp2 = exp2 ()
 in (
A.ArrayExp{typ = A.symbol ID, size = exp1, init = exp2,              pos = IDleft}
)
end; ()))
...
|  ( 29, ( ( _, ( _, _, END1right)) :: ( _, ( MlyValue.ntVOID expseq1,
 _, _)) :: _ :: ( _, ( MlyValue.ntVOID declarationlist1, _, _)) :: ( _
, ( _, (LETleft as LET1left), _)) :: rest671)) => let val  result = 
MlyValue.ntVOID (fn _ => ( let val  (declarationlist as 
declarationlist1) = declarationlist1 ()
 val  (expseq as expseq1) = expseq1 ()
 in (
A.LetExp{decs = declarationlist, body = expseq,                      pos = LETleft}
)
end; ()))
 in ( LrTable.NT 0, ( result, LET1left, END1right), rest671)
end
|  ( 30, ( ( _, ( _, _, RPAREN1right)) :: _ :: ( _, ( _, LPAREN1left,
 _)) :: rest671)) => let val  result = MlyValue.ntVOID (fn _ => (ERROR
))
 in ( LrTable.NT 0, ( result, LPAREN1left, RPAREN1right), rest671)
end
...
lvalue1) = lvalue1 ()
 val  (ID as ID1) = ID1 ()
 in (A.FieldVar{lvalue, A.symbol(ID), lvalueleft} )
end; ()))
...
lvalue1left), _)) :: rest671)) => let val  result = MlyValue.ntVOID
 ...
 val  exp1 = exp1 ()
 in (A.SubscriptVar{lvalue, A.symbol(ID), lvalueleft})
end; ()))
 in ( LrTable.NT 2, ( result, lvalue1left, RBRACK1right), rest671)
end
|  ( 34, ( ( _, ( MlyValue.ntVOID exp1, exp1left, exp1right)) :: 
rest671)) => let val  result = MlyValue.ntVOID (fn _ => ( let val  (
exp as exp1) = exp1 ()
 in (exp :: []])
end; ()))
...
|  ( 37, ( ( _, ( MlyValue.ntVOID exp1, exp1left, exp1right)) :: 
rest671)) => let val  result = MlyValue.ntVOID (fn _ => ( let val  (
exp as exp1) = exp1 ()
 in (exp :: [])
end; ()))
...
result = MlyValue.ntVOID (fn _ => ( let val  (explist as explist1) = 
explist1 ()
 val  (exp as exp1) = exp1 ()
 in (exp :: explist)
end; ()))
...
|  ( 40, ( ( _, ( MlyValue.ntVOID declaration1, declaration1left, 
...
|  ( 41, ( ( _, ( MlyValue.ntVOID declaration1, _, declaration1right))
...
|  ( 42, ( ( _, ( MlyValue.ntVOID tyDecs1, tyDecs1left, tyDecs1right))
 :: rest671)) => let val  result = MlyValue.ntVOID (fn _ => ( let val 
...
|  ( 44, ( ( _, ( MlyValue.ntVOID funDecs1, funDecs1left, 
...
rest671)) => let val  result = MlyValue.ntVOID (fn _ => ( let val  (ID
 ...
 val  (exp as exp1) = exp1 ()
 in (
A.VarDec{name=A.symbol(ID), escape=ref false,
...
rest671)) => let val  result = MlyValue.ntVOID (fn _ => ( let val  (ID
 ...
 val  (exp as exp1) = exp1 ()
 in (
A.fundec{name = A.symbol ID, params = typefields,
															result = NONE,
															body = exp,
															pos = FUNCTIONleft}
)
...
MlyValue.ID ID2, ID2left, _)) :: _ :: _ :: ( _, ( MlyValue.ntVOID 
...
, (FUNCTIONleft as FUNCTION1left), _)) :: rest671)) => let val  result
 ...
 val  (exp as exp1) = exp1 ()
 in (
A.fundec{name = A.symbol ID1, params = typefields,
...
, _, _)) :: ( _, ( _, (FUNCTIONleft as FUNCTION1left), _)) :: rest671)
 ...
 val  (exp as exp1) = exp1 ()
 in (
A.fundec{name = A.symbol ID1, params = [],
															result = SOME(A.symbol ID2, ID2left),
															body = exp,
															pos = FUNCTIONleft}
)
...
|  ( 51, ( ( _, ( MlyValue.ntVOID functiondeclaration1, 
...
functiondeclaration as functiondeclaration1) = functiondeclaration1 ()
 ...
 in ( LrTable.NT 14, ( result, functiondeclaration1left, 
functiondeclaration1right), rest671)
end
|  ( 52, ( ( _, ( MlyValue.ntVOID functiondeclaration1, _, 
functiondeclaration1right)) :: ( _, ( MlyValue.ntVOID funDecs1, 
funDecs1left, _)) :: rest671)) => let val  result = MlyValue.ntVOID
...
functiondeclaration1 ()
 ...
 in ( LrTable.NT 14, ( result, funDecs1left, functiondeclaration1right
), rest671)
end
...
typefields1, _, _)) :: ( _, ( _, LBRACE1left, _)) :: rest671)) => let
 val  result = MlyValue.ntVOID (fn _ => ( let val  (typefields as 
...
|  ( 60, ( ( _, ( _, _, RBRACE1right)) :: ( _, ( _, LBRACE1left, _))
 :: rest671)) => let val  result = MlyValue.ntVOID (fn _ => (
A.RecordTy([])))
 in ( LrTable.NT 16, ( result, LBRACE1left, RBRACE1right), rest671)
...
MlyValue.ntVOID (fn _ => ( let val  (ID as ID1) = ID1 ()
 in (A.ArrayTy{name=A.symbol(ID), pos=ARRAYleft})
end; ()))
 in ( LrTable.NT 16, ( result, ARRAY1left, ID1right), rest671)
end
|  ( 62, ( ( _, ( MlyValue.ntVOID typefield1, typefield1left, 
typefield1right)) :: rest671)) => let val  result = MlyValue.ntVOID
 (fn _ => ( let val  (typefield as typefield1) = typefield1 ()
 in (typefield :: [])
end; ()))
 in ( LrTable.NT 12, ( result, typefield1left, typefield1right), 
rest671)
end
|  ( 63, ( ( _, ( MlyValue.ntVOID typefield1, _, typefield1right)) ::
 _ :: ( _, ( MlyValue.ntVOID typefields1, typefields1left, _)) :: 
rest671)) => let val  result = MlyValue.ntVOID (fn _ => ( let val  (
typefields as typefields1) = typefields1 ()
...
MlyValue.ntVOID (fn _ => ( let val  ID1 = ID1 ()
 val  ID2 = ID2 ()
 in (
A.field{name = A.symbol ID1, escape = ref true, typ = A.symbol ID2, pos = ID1left}
)
end; ()))
